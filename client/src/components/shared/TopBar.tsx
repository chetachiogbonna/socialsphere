...
function TobBarMenu({ setIsUserLoggingOut }: { setIsUserLoggingOut: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  return (
    <div className="md:w-52">
      <div>
        <div 
          className="flex justify-center items-end gap-3 p-1"
        >
          <div 
          ...
          >
            <img src={user?.profilePicUrl || "/assets/images/profile-placeholder.jpg"} className="w-8 h-8 rounded-full" alt="" />
            <img src={`/assets/icons/chevron-${toggleProfileMenu ? "up" : "down"}.png`} className="w-8 h-8 rounded-full" alt="" />
          </div>
      </div>
      </div>
  ...
  return (
    <header className="bg-[#1A1A1A] z-10 h-[60px] mb-4 fixed top-0 right-0 left-0">
      <div className="max-w-screen-2xl mx-auto flex justify-between items-center">
        ...
        <p className="w-[98%] md:w-[80%] hidden md:block mx-auto text-[8px] text-center text-gray-400">Tips: (Navigation to other pages with AI is general every page has it.)</p>
